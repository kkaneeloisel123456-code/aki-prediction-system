...
export interface FeatureMeta { name: string; median: number; mean: number; std: number; min: number; max: number; timing: string }
export interface FeaturesResponse { features: string[]; metas: FeatureMeta[]; risk_low: number; risk_high: number }
export interface PredictResponse { patient_id: string | null; probability: number; prediction: number; risk_level: '低'|'中'|'高'; calibrated: boolean; shap_values: ShapContribution[]; expected_value: number }
export interface PerformanceResponse { cv?: Record<string, unknown>[]; calibration?: unknown[]; summary?: unknown[] }
export interface MetaResponse { n_features: number; n_models: number; best_auc: number | null; risk_low: number; risk_high: number }
export interface CohortPatient { id: string; age: number; sex: string; surgery: string; preScr: number; preEgfr: number; apache: number; probability: number; riskLevel: string }
