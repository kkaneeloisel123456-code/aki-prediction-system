...
def count_selected_features(selector, X_sample):
    ...
    if hasattr(selector, "get_support"):
        try:
            ...
            pass
    if (
        hasattr(selector, "support_")
        and getattr(selector, "support_", None) is not None
    ):
        return int(np.asarray(selector.support_).sum())
    if (
        hasattr(selector, "n_features_")
        and getattr(selector, "n_features_", None) is not None
    ):
...
class BlendingClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X):
        X = np.asarray(X)
        p = (
            np.column_stack([m.predict_proba(X)[:, 1] for m in self.base_models_])
            @ self.weights_
        )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="快速版（减少折数与 trials）"
    )
    ...
    parser.add_argument(
        "--tune-trials", type=int, default=20, help="每个外折的 Optuna trials"
    )
    parser.add_argument(
        "--fs-repeats", type=int, default=2, help="特征选择对比重复次数"
    )
    parser.add_argument(
        "--interaction-repeats", type=int, default=3, help="交互项对比重复次数"
    )
    ...
    data_path = ROOT / "data" / "raw" / "AKI数据.xlsx"
    if not data_path.exists():
        print(f"[ERROR] 数据文件不存在: {data_path}")
        print("请将原始 AKI 数据放置到 data/raw/ 目录下。")
        return
    ...
    fixed_df.to_csv(
        OUT_TABLES / "advanced_fixed_cv.csv", index=False, encoding="utf-8-sig"
    )
    ...
    tune_df.to_csv(
        OUT_TABLES / "advanced_tuning_summary.csv", index=False, encoding="utf-8-sig"
    )
    ...
    for sel_name in selectors:
        ...
        fs_rows.append(
            summarize_scores(
                sel_name,
                aucs,
                中位特征数=float(np.median(n_feats)),
            )
        )
        print(
            f"  {sel_name}: AUC={np.mean(aucs):.4f} ± {np.std(aucs):.4f}, 中位特征={np.median(n_feats)}"
        )
    fs_df = pd.DataFrame(fs_rows)
    fs_df.to_csv(
        OUT_TABLES / "advanced_feature_selection.csv", index=False, encoding="utf-8-sig"
    )
    ...
    pairs_available = [
        (a, b) for a, b in interaction_pairs if a in X.columns and b in X.columns
    ]
    ...
    for label, interactions in [
        ...
        for tr_idx, te_idx in cv_inter.split(X, y):
            ...
            pipe.fit(X_tr, y_tr)
            n_feats.append(
                count_selected_features(pipe.named_steps["selector"], X_tr.iloc[:1])
            )
            aucs.append(roc_auc_score(y_te, pipe.predict_proba(X_te)[:, 1]))
    ...
    inter_df.to_csv(
        OUT_TABLES / "advanced_interactions.csv", index=False, encoding="utf-8-sig"
    )
