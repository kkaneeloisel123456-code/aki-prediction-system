...
**折内预处理**：CV 使用未做全局填补的原始候选矩阵（49 个缺失值 = 420×87 候选矩阵经临床范围校验、置不可能值为缺失后的缺失值总数，对应 `outputs/tables/clinical_range_flags.csv` 的 44 条标记，该行级明细在受控环境生成、不随公开仓库分发），中位数填补、标准化与 RF 特征筛选均在每个训练折内完成，避免“用全量数据算中位数再进 CV”的隐含泄漏。
