# AKI 急性肾损伤智能预测系统

> **在线体验**：[https://aki-prediction-system.onrender.com/](https://aki-prediction-system.onrender.com/)（免费实例冷启动可能需要几十秒）

## 项目简介

基于广西某三甲医院临床数据（420例），利用多种机器学习算法构建急性肾损伤（AKI）预测模型。通过 SHAP 可解释性分析 + DCA 决策曲线 + 嵌套式交叉验证 + Bootstrap 内部验证，为临床决策提供透明、可信的个体化风险评估。最终部署为 FastAPI + Vue 3 的在线临床决策支持系统。

---

## 快速启动

需要 Python 3.10+（仅前端需要重新构建时才要求 Node.js 18+）。

### 方式一：一键启动（Windows 推荐）

双击项目根目录的 `启动系统.bat`，脚本会自动完成：查找/创建虚拟环境 → 安装后端依赖 → 安装前端依赖 → 构建前端（dist 未过时自动跳过）→ 启动服务并自动打开浏览器。关闭窗口即停止服务。若 8000 端口被本系统的旧实例占用，脚本会自动结束旧实例并继续启动；若 8000 被占用，自动切换到 8001-8010 备用端口。

### 方式二：手动启动

终端进入项目文件夹后执行下面操作。

```bash
# 1. 后端依赖
python -m venv .venv
.venv\Scripts\activate            # Windows
pip install -r backend/requirements.txt

# 2. 前端依赖与构建
cd frontend
npm install
npm run build                     # 产物输出到 frontend/dist
cd ..

# 3. 启动服务（后端同时托管前端静态页面）
uvicorn backend.app.main:app --host 0.0.0.0 --port 8000
```

浏览器访问 [http://localhost:8000](http://localhost:8000) 。首次启动会从 `app_data/` 加载模型工件（已随包提供，无需 Git LFS）。

开发模式（前端热重载）：另开一个终端进入 `frontend` 目录后执行 `npm run dev`，访问 [http://localhost:5173](http://localhost:5173) ，Vite 会把 `/api` 代理到 8000 端口的后端。

---

## 模型性能

| 指标                                            | 数值                                                   |
| ----------------------------------------------- | ------------------------------------------------------ |
| **5折×10次=50次嵌套CV AUC**                     | **0.8096 ± 0.0428**                                    |
| 测试集 AUC                                      | 0.8007                                                 |
| Bootstrap AUC（对5折OOF概率按患者重采样1000次） | 0.799 [0.754, 0.842]                                   |
| 校准后 Brier（基于5折OOF概率拟合）              | 0.168（原始 0.180）                                    |
| 过拟合差距（单次20%切分参考值, seed 42）        | 0.129；Bootstrap 区间 [0.033, 0.249]                   |
| 最佳模型                                        | Voting Ensemble (LR:2, RF:2, XGB:1, ET:1)              |
| 使用特征                                        | 35 个 (RF重要性Top35)                                  |
| 验证方式                                        | RepeatedStratifiedKFold 嵌套式 (5折 × 10次 = 50次评估) |

| 模型                | 50次CV AUC | 标准差    |
| ------------------- | ---------- | --------- |
| LogisticRegression  | 0.794      | 0.043     |
| RandomForest        | 0.807      | 0.041     |
| XGBoost             | 0.806      | 0.044     |
| ExtraTrees          | 0.794      | 0.043     |
| **Voting Ensemble** | **0.809**  | **0.043** |

---

## 高级方法对比（Wave 1）

新增 `run_advanced.py`，与 `run_clean.py` 共用同一套数据准备与泄漏规则。所有实验（调参、特征选择、交互项、Stacking/Blending）都在训练折内完成，统一用嵌套 CV 报告。

| 配置                                 | 25折AUC             | 说明           |
| ------------------------------------ | ------------------- | -------------- |
| **Voting (LR:2, RF:2, XGB:1, ET:1)** | **0.8070 ± 0.0433** | 当前最终模型   |
| LogisticRegression                   | 0.7928 ± 0.0457     | 固定正则化配置 |
| RandomForest                         | 0.8051 ± 0.0376     | 固定正则化配置 |
| XGBoost                              | 0.7999 ± 0.0435     | 固定正则化配置 |
| ExtraTrees                           | 0.7950 ± 0.0396     | 固定正则化配置 |
| Stacking                             | 0.7741 ± 0.0415     | 未超过 Voting  |
| OOF加权 Blending（5折OOF）           | 0.7994 ± 0.0402     | 未超过 Voting  |

> 注：上表为 25 折辅助口径（5折×5次嵌套CV）；主口径为 50 次嵌套CV（见模型性能表，Voting 0.8096 ± 0.0428）。

### Optuna 贝叶斯调参（同一5折上对比，每折20 trials，内层3折CV）

| 模型               | 固定配置 AUC | Optuna 调优 AUC |
| ------------------ | ------------ | --------------- |
| LogisticRegression | 0.7890       | 0.7862          |
| RandomForest       | 0.8000       | 0.7942          |
| XGBoost            | 0.7863       | 0.7642          |
| ExtraTrees         | 0.7955       | 0.7943          |
| Voting             | 0.8020       | 保持固定权重    |

**结论**：420例/125事件下，Optuna 搜索并未显著超过手工强正则化配置；Voting 集成仍是最优，因此不更换最终模型，但该对比本身可作为方法严谨性证据写入报告。

### 特征选择与交互项

| 特征方案             | AUC（10折）     | 中位特征数 |
| -------------------- | --------------- | ---------- |
| RF Top35（当前方案） | 0.8091 ± 0.0377 | 35         |
| RF Top20             | 0.8006 ± 0.0380 | 20         |
| RFECV                | 0.8060 ± 0.0336 | 67         |
| Boruta-lite          | 0.8008 ± 0.0340 | 16         |

| 方案                 | AUC（15折）     |
| -------------------- | --------------- |
| 无交互项（当前方案） | 0.8065 ± 0.0484 |
| 6个临床交互项        | 0.8070 ± 0.0476 |

**结论**：交互项增益约 +0.0005，RFECV 与 Top35 相当但特征更多。考虑到 EPV=3.6，保持当前 35 特征方案不变。

运行：`python run_advanced.py`

---

## Wave 2：填补 / 过采样 / 阈值 / 不确定性

新增 `run_wave2.py`。与 Wave 1 的差异是：Wave 2 使用**未做全局中位数填补的原始候选矩阵**，所有填补、缩放、筛选、过采样都在训练折内完成，避免隐含的全局填补泄漏。

### MICE 与中位数填补对比（5折 x 3次，训练折内拟合）

| 填补方案                 | AUC             | Brier  |
| ------------------------ | --------------- | ------ |
| 中位数填补（当前方案）   | 0.8065 ± 0.0484 | 0.1784 |
| MICE（IterativeImputer） | 0.8052 ± 0.0473 | 0.1791 |

结论：MICE 未带来增益，中位数填补在 420 例规模下更简单、更稳定，保留现状。

### SMOTE 与 class_weight 对比（5折 x 3次）

| 配置                               | AUC             | Brier  |
| ---------------------------------- | --------------- | ------ |
| XGB + scale_pos_weight（当前）     | 0.7967 ± 0.0491 | 0.1701 |
| XGB + SMOTE（无权重）              | 0.7861 ± 0.0439 | 0.1768 |
| LR + class_weight=balanced（当前） | 0.7958 ± 0.0492 | 0.1842 |
| LR + SMOTE（无权重）               | 0.7929 ± 0.0460 | 0.1845 |

结论：SMOTE 未超过 class_weight / scale_pos_weight，保留现有类别权重策略。

### DCA 阈值推荐与成本效益（Wave2 口径：run_wave2.py，5折OOF未校准概率）

- 推荐决策阈值：**0.29**（“模型净获益 − max(全部干预净获益, 0)”差值最大点；该阈值下模型净获益 0.097、全部干预净获益 0.011）
- OOF AUC（5折）：0.7967

| 干预成本/AKI成本 | 对应阈值 | 每千例真阳 | 每千例假阳 | 净获益差值/千例 |
| ---------------- | -------- | ---------- | ---------- | --------------- |
| 0.1              | 0.091    | 297.6      | 692.9      | +0.95           |
| 0.2              | 0.167    | 295.2      | 604.8      | +17.14          |
| 0.3              | 0.231    | 290.5      | 497.6      | +54.29          |
| 0.5              | 0.333    | 261.9      | 373.8      | +128.57         |
| 1.0              | 0.500    | 211.9      | 181.0      | +435.71         |

表格以“干预成本/AKI成本”比例表达，不虚构绝对费用；报告使用时需说明为假设性敏感性分析。

> **口径说明**：本表为 Wave2 DCA 口径（未校准概率，以“模型净获益 − 全部干预净获益”差值最大化选点，推荐 0.29）；与下方 P1 实验 DCA 表（净获益最大化选点，推荐 0.20–0.25）口径不同，两表不直接比较。

### 重复 CV 不确定性量化（5折 x 10次 = 50次）

| 指标                                               | 数值                  |
| -------------------------------------------------- | --------------------- |
| 概率均值 AUC                                       | 0.8079                |
| 95% 区间宽度均值                                   | 0.100                 |
| 95% 区间宽度中位数                                 | 0.097                 |
| 区间宽度 > 0.15 比例                               | 10.95%                |
| 低 / 中 / 高风险比例（未校准概率，0.30/0.70 分层） | 31.2% / 56.9% / 11.9% |

患者级预测区间在受控环境生成（`outputs/tables/wave2_uncertainty_patients.csv`），按仓库隐私约定不随公开仓库分发；报告可使用汇总统计或去标识化展示。

> **Web 风险分层**：Web 端以**校准后概率**分层（`src/config.py`：RISK_LOW=0.30、RISK_HIGH=0.70，即 低<0.30 / 中0.30–0.70 / 高≥0.70），用于风险展示；DCA 推荐阈值（0.29 未校准 / 0.20–0.25 校准后）用于干预决策，两者用途不同、不直接比较。

---

## P1 补充实验：临床基线 / 精简模型 / DCA / TabNet

新增 `run_p1.py`，与 `run_clean.py` 共用同一套数据准备与泄漏规则；各实验使用 `prepare_training_data` 全局中位数填补矩阵（与 run_clean 模块4 的折内填补路径不同，具体口径见各表注），CV 内无目标泄漏，诚实报告（无选择偏倚）。

### 1）临床基线对比（Thakar / STS 简化评分）

| 模型                | OOF AUC（5折） | 95%CI       | 与Voting的DeLong P |
| ------------------- | -------------- | ----------- | ------------------ |
| Thakar 简化评分     | 0.5225         | 0.466–0.574 | <0.0001            |
| STS 简化评分        | 0.5637         | 0.503–0.621 | <0.0001            |
| 临床LR（术前+术中） | 0.7185         | 0.662–0.767 | 0.0001             |
| **Voting 全模型**   | **0.8019**     | 0.754–0.847 | —                  |

- 标准 DeLong 检验（含协方差）：Z = -7.89 / -6.91 / -3.93，Voting 均显著优于经典评分与临床 LR 基线。
- 说明：原始 Thakar/STS 评分需完整变量，本研究使用文献公式的"简化复现版"（口径透明、可复现），报告建议同样表述。
- 注：本表 AUC 为 5 折 OOF 口径（Voting 0.8019），与主口径 50 次嵌套CV（0.8096）属不同验证方案，差异属正常范围。

### 2）精简特征模型（Top8 / Top12）

| 特征数     | 模型  | 50次CV AUC均值 ± 标准差 |
| ---------- | ----- | ----------------------- |
| 8          | LR    | 0.8055 ± 0.0425         |
| 8          | Ridge | 0.8038 ± 0.0448         |
| 12         | LR    | 0.8022 ± 0.0417         |
| 12         | Ridge | 0.7996 ± 0.0452         |
| 35（当前） | LR    | 0.8119 ± 0.0441         |
| 35（当前） | Ridge | 0.8046 ± 0.0426         |

- 结论：Top8 LR（0.8055）与 35 特征 LR（0.8119）差距约 0.006；考虑 EPV=3.6，精简方案可作为临床部署备选，主模型仍保留 35 特征 Voting。

### 3）DCA 阈值网格与成本效益（P1 口径：run_p1.py，每千例）

| 阈值     | 灵敏度    | 特异度    | 净获益     | 每千例预测高危 | 每千例正确识别AKI |
| -------- | --------- | --------- | ---------- | -------------- | ----------------- |
| 0.10     | 0.976     | 0.342     | 0.2392     | 752.4          | 290.5             |
| 0.15     | 0.976     | 0.342     | 0.2090     | 752.4          | 290.5             |
| **0.20** | **0.872** | **0.583** | **0.1863** | 552.4          | 259.5             |
| **0.25** | **0.856** | **0.607** | **0.1627** | 531.0          | 254.8             |
| 0.30     | 0.840     | 0.624     | 0.1367     | 514.3          | 250.0             |

> 注：阈值 0.10 与 0.15 行数值相同，原因为该概率区间内无患者样本（Isotonic 校准后概率分布离散化所致）。

- 推荐操作阈值 **0.20–0.25**：灵敏度≈0.86–0.87、特异度≈0.58–0.61，每千例约正确识别 255–260 例 AKI，同时避免对约半数人群过度干预。
- 阈值 0.05–0.10 净获益虽高，但会把 75–81% 患者标记为高危，临床不可操作；完整网格见 `outputs/p1/dca_threshold_cost.csv`。

> **口径说明**：本表为 P1 DCA 口径（run_p1.py，5折OOF、**校准后概率**、净获益最大化选点）；与上方 Wave2 DCA 表（0.29，未校准概率、差值最大化选点）口径不同，两表不直接比较。

### 4）TabNet 诚实实验（5折×5次=25次，同折对比）

| 模型               | AUC均值 ± 标准差    |
| ------------------ | ------------------- |
| TabNet             | 0.7857 ± 0.0408     |
| **Voting（同折）** | **0.8135 ± 0.0397** |

> **口径说明**：本表 Voting AUC（0.8135）与 Wave 1 固定配置表（0.8070）的差异源于输入矩阵不同——run_p1.py 使用全局中位数填补后的矩阵（`prepare_training_data['X']`），run_advanced.py 使用保留缺失值的原始矩阵并在每折内填补/缩放/筛选（与 run_clean.py 模块4 的折内预处理一致）。两者均为诚实 CV（无目标泄漏），仅预处理路径不同，Voting 相对排名结论一致；本表与主口径 0.8096（折内预处理 + 50次嵌套CV）亦属不同验证方案，不直接比较。

- 结论：小样本（420例/125事件）下 TabNet 未优于 Voting（差约 0.028），不引入深度模型复杂度；该负面结果作为方法学严谨性证据。

### 5）特征获取时点 vs 预测时点对照表

- 新增 `outputs/tables/feature_timing.csv`：35 个最终特征逐项给出代码时段（术前/术中/ICU入室/术后早期）、最早可得时点与临床说明。
- 全部特征在 AKI 诊断窗（术后48h/7d）之前可得；预测时点可理解为"入ICU即刻抽血、检验回报后（约1–2小时）可预测"（部分术后早期检验需30min–2h回报，详见下表"最早可得时点"）。该表用于回应"特征时序/临床可操作性"评审问题（报告"特征工程"模块建议引用）。

运行：`python run_p1.py`

---

## 特征方案

| 类别             | 数量   | 说明                                                                                   |
| ---------------- | ------ | -------------------------------------------------------------------------------------- |
| 术前特征+人口学  | 44     | 人口学、实验室检查、生命体征、手术类型                                                 |
| 术中特征         | 4      | 失血量、输液量、尿量                                                                   |
| ICU入室值        | 2      | 入ICU即刻eGFR、肌酐                                                                    |
| 术后早期非肌酐   | 33     | 血常规、炎症标志物、血气                                                               |
| **候选建模特征** | **87** | 97列→排除14列→83原始候选列；手术类型One-Hot后共87个数值特征                            |
| **精筛后**       | **35** | RF重要性筛选 Top35                                                                     |
| **已排除**       | **14** | 13个泄漏列（身份字段+KDIGO诊断标准含术后尿素+结局变量+术后7d+通气时间）+ 目标列AKI分组 |

### 数据泄漏控制

初始模型 AUC > 0.99，经审查发现特征中包含术后48h/7d肌酐——这正是 KDIGO 急性肾损伤诊断标准本身（"用答案预测答案"）。团队进行了系统性修复：

| 排除类别             | 示例                                           | 原因                                                |
| -------------------- | ---------------------------------------------- | --------------------------------------------------- |
| 身份字段             | 姓名、住院号                                   | 隐私与个体识别，禁止入模                            |
| KDIGO诊断标准        | 术后48hSCr, 术后48heGFR, 术后7dSCr, 术后7deGFR | 诊断标准本身                                        |
| 术后尿素（答案近亲） | 术后48hUrea, 术后7dUrea                        | 与肌酐同步升高，非KDIGO标准但属“答案近亲”，一并排除 |
| 结局变量             | 总住院天数, 总住院费用, ICU住院天数            | 预测时点之后的结局                                  |
| 术后7天指标          | 术后7dRBP                                      | AKI在7天内已确诊                                    |
| 术后通气时间         | 术后通气时间                                   | 接近结局变量                                        |

**保留特征论证**: 所有保留特征在AKI诊断（术后48h/7d）之前即可获取。预测时点为"入ICU即刻抽血、检验回报后（约1–2小时）可预测"，远早于AKI诊断窗，临床可操作性强。

**折内预处理**：CV 使用未做全局填补的原始候选矩阵（49 个缺失值 = 44 条临床范围校验标记（置不可能值为缺失） + 5 个原始数据自带缺失（未纳入范围校验字典），对应 `outputs/tables/clinical_range_flags.csv` 的 44 条标记，该行级明细在受控环境生成、不随公开仓库分发），中位数填补、标准化与 RF Top35 特征选择（SelectFromModel 每折重新拟合）均在每个训练折内完成，避免“用全量数据算中位数/选特征再进 CV”的隐含泄漏；模块3 基于全量数据确定的 Top35 仅用于最终部署特征清单与 SHAP/相关性等解释分析，不参与 CV 性能评估。

| 阶段                    | AUC               | 验证                    | 说明                                     |
| ----------------------- | ----------------- | ----------------------- | ---------------------------------------- |
| 初始模型（含泄漏）      | >0.99             | 单次划分                | 用KDIGO标准预测AKI — 不可信              |
| 纯术前（保守）          | 0.74              | 5折CV                   | 仅术前特征，完全无泄漏（历史对比）       |
| 术前+术中+ICU入室       | 0.79              | 5折CV                   | 临床可论证的预测时点（历史对比）         |
| **最终版（+术后早期）** | **0.8096±0.0428** | **5折×10次=50次嵌套CV** | **当前最优，填补/缩放/筛选均在训练折内** |

> **"一个可信的 0.81，胜过一百个泄漏的 0.99。"**

---

## 项目结构

```text
aki-prediction-system/
├── 启动系统.bat                  # Windows 一键启动（venv/依赖/构建/启动全自动）
├── data/                         # 数据（不随公开仓库分发）
│   └── raw/AKI数据.xlsx          # 原始数据（受控环境）
├── models/                       # 训练产物（Git-LFS 指针）
│   ├── final_voting_model.pkl    # 最终 Voting 模型
│   ├── {LogisticRegression,RandomForest,XGBoost,ExtraTrees}.pkl
│   ├── calibrator.pkl / scaler.pkl
│   └── selected_features.txt     # 选中的 35 个特征
├── app_data/                     # 部署工件（普通 Git 文件，后端实际加载）
│   ├── final_model.joblib        # Voting 模型（~2.4MB）
│   ├── scaler.joblib / calibrator.joblib
│   ├── features.txt              # 35 特征名（与 models/ 一致）
│   └── impute_values.json        # 推理时缺失值中位数
├── backend/                      # FastAPI 后端
│   ├── app/                      # main / predictor / assets / pdf / schemas / config
│   ├── assets/fonts/             # PDF 用 CJK 字体
│   └── requirements.txt
├── frontend/                     # Vue 3 + Vite + TypeScript 前端
│   ├── src/                      # 页面、组件、API 客户端、composables
│   ├── dist/                     # 构建产物（由后端托管，不入库）
│   └── package.json
├── outputs/                      # 训练输出
│   ├── figures/                  # ROC/PR/DCA/校准/SHAP/消融/亚组图
│   ├── tables/                   # CV/Bootstrap/HL/SHAP/特征时点等表
│   └── p1/                       # P1 补充实验（临床基线/TopN/DCA/TabNet）
├── src/                          # ML 源代码
│   ├── config.py                 # 泄漏规则、风险阈值
│   ├── data/prepare.py           # 数据清洗、范围校验、one-hot、中位数填补
│   ├── models/                   # 校准、时序等模块
│   └── visualization/            # 图表模块（含 data_governance）
├── tests/                        # unittest 一致性与防泄漏回归测试
├── run_clean.py                  # ★ 主管线（5折×10次重复分层交叉验证 ）
├── run_advanced.py               # Wave1：Optuna/Stacking/特征选择/交互项
├── run_wave2.py                  # Wave2：MICE/SMOTE/阈值/不确定性
├── run_p1.py                     # P1：临床基线/TopN/DCA/TabNet
├── run_evaluation.py             # 出版级图表
├── run_bonus.py                  # VIF/PDP/三方法交叉验证/HL
├── requirements.txt              # 训练侧依赖（后端用 backend/requirements.txt）
└── README.md
```

---

## 方法论完整度

| 方法                | 说明                                                                                                                                   | 结论                                                                                                       |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| **三方法交叉验证**  | Logistic（Top12显著特征）+ XGBoost（Top10重要性）+ SHAP（Top10重要性）独立验证；三种特征口径不同，仅用于特征稳健性互证，不比较预测性能 | 8个核心特征被≥2种方法确认                                                                                  |
| **PDP非线性分析**   | 部分依赖图展示剂量-效应关系                                                                                                            | 核心特征呈现阈值加速、开关效应                                                                             |
| **亚组分析**        | 按风险/肾功能分层评估（使用5折OOF预测，避免训练集内自评）                                                                              | 高风险组AKI率48.6%，低风险组11.0%（约4.4倍差距）                                                           |
| **VIF共线性诊断**   | 方差膨胀因子检验（标准化矩阵+截距）                                                                                                    | VIF>10共6项（最高ICUAdmeGFR=36.5），RF筛选+L2正则化缓解，解释时需注意                                      |
| **Hosmer-Lemeshow** | 拟合优度检验                                                                                                                           | P=0.149（在20%测试集上以LR简化模型近似复现，n≈84；样本量小，仅供参考；整体预测概率水平偏高，详见校准限制） |

> **校准说明**：原始模型在 OOF 上期望阳性合计约181.4，而实际阳性为125，说明未校准概率整体偏高。系统已内置 Isotonic 校准（基于5折OOF概率拟合），校准后 Brier 由 0.180 降至 0.168；Web 端展示的即为校准后概率，SHAP 解释仍基于原始模型。
>
> **图口径说明**：roc_curves.png 标注的 AUC 为 50 次嵌套CV均值、曲线基于 5 折 OOF；dca_curve.png / dca_with_ci.png / decision_curve.png 为同一 DCA 图（run_clean 5折OOF，未校准概率）；wave2_threshold_nb.png 为 run_wave2 口径（未校准5折OOF，0.29 选点）；outputs/tmp_gauge.png 为 Web PDF 报告运行时生成的临时仪表图（非评审输出，已加入 .gitignore）。
> **特征选择口径说明**：`run_evaluation.py` 与 `run_wave2.py` 的 `SelectFromModel` 已与 `run_clean.py` 对齐（`threshold=-np.inf`，每折固定 35 特征），评估图表均基于与最终模型一致的 35 特征矩阵。
> **时序模块口径说明**：`outputs/phase3/tables/temporal_results.csv` 的 AUC 为**单次 5 折 cross_val_score 按折均值**（T2=0.816±0.0263），与主口径 50 次重复嵌套CV（0.8096）或 OOF 合并概率口径（≈0.80）**不是同一验证方案，不直接比较**；该表仅用于展示"信息累积带来的 AUC 增益趋势"。

运行：`python run_bonus.py`

> **注意**：若克隆仓库后 run_evaluation/run_bonus 报 KeyError: 118，说明 models/*.pkl 是 Git LFS 指针。请先 `git lfs pull` 或运行 `python run_clean.py` 重新训练。

---

## 快速开始

### 启动 Web 系统

见顶部"快速启动"。Windows 下双击 `启动系统.bat` 一键启动；手动方式简而言之：

终端进入项目文件夹后执行下面操作：

```bash
pip install -r backend/requirements.txt
cd frontend
npm install
npm run build
cd ..
uvicorn backend.app.main:app --host 0.0.0.0 --port 8000
# → http://localhost:8000
```

浏览器访问 [http://localhost:8000](http://localhost:8000) 。

### 重新训练 / 复现实验（需要受控环境中的原始数据）

```bash
# 训练侧依赖（后端依赖是其子集）
pip install -r requirements.txt

# 1. 主管线：特征筛选 → 训练 → 50次嵌套CV → 过拟合检查 → 保存模型/图/表
python run_clean.py

# 2. 出版级比赛图表（ROC/PR/校准/DCA/SHAP）
python run_evaluation.py

# 3. （VIF/PDP/三方法交叉验证/HL）
python run_bonus.py

# 4. 高级方法对比（Optuna/Stacking/特征选择/交互项）
python run_advanced.py

# 5. Wave 2：MICE/SMOTE/阈值/不确定性
python run_wave2.py

# 6. P1 补充实验：临床基线/TopN/DCA/TabNet
python run_p1.py

# 7. 时序风险轨迹（T0→T1→T2）
python -m src.models.temporal

# 8. 一致性回归测试
python -m unittest discover -s tests -v
```

> `data/raw/AKI数据.xlsx` 含患者可识别信息，**不随公开仓库分发**；上述训练脚本需要在受控环境获取该文件后运行。Web 系统加载的是 `app_data/` 下已训练好的模型工件，可独立启动。

---

## 评价体系

| 维度           | 方法                                                                |
| -------------- | ------------------------------------------------------------------- |
| **区分度**     | AUC-ROC, Precision-Recall AUC, 5折×10次=50次嵌套CV, Bootstrap 95%CI |
| **校准度**     | Brier Score, Calibration Curve, OOF Isotonic 校准                   |
| **临床效用**   | Decision Curve Analysis (DCA)                                       |
| **可解释性**   | SHAP Summary/Bar/Force/Dependence Plot                              |
| **过拟合控制** | 训练-测试AUC差距, 加强正则化                                        |
| **综合指标**   | Accuracy, Precision, Recall, F1, Specificity                        |

---

## 答辩要点（证据定位）

> **主证据**：50 次重复嵌套CV 的 AUC（0.8096 ± 0.0428）、Bootstrap 95% CI 与校准曲线（Isotonic 校准后 Brier 0.168）。三者来自同一 35 特征 + 严格折内预处理的完整流程，口径统一、可复现、无信息泄漏。
>
> **辅助证据**：HL 检验（P=0.149，20% 测试集近似复现，n≈84）、Brier、DCA 阈值推荐定位为方法学严谨性与临床实用性的补充说明，不作为性能主指标；其口径差异（如 Wave2 与 P1 的 DCA 选点差异、HL 为近似复现）已在对应表格注明。
>
> **答辩叙事**：强调"0.81 的可信 AUC 胜过泄漏的 0.99"——报告内所有数字均来自严格折内预处理与嵌套CV；任何更高的数字若未说明验证口径与防泄漏措施，都不具备可比性。

---

## Web 系统功能

系统为 FastAPI 后端 + Vue 3 前端的单页应用，共 8 个页面：

1. **首页**: 核心指标（AUC/样本量/发生率）、模型对比、功能导航
2. **模型性能**: 50次嵌套CV结果表、混淆矩阵、ROC/PR、校准/DCA、Bootstrap分布、消融热力图、集成对比
3. **风险预测**（核心）: 分时段Tab表单（术前/术中/ICU/术后）→ 实时预测 → 风险仪表盘 → SHAP Top6双向条形图 → PDF报告下载
4. **医生工作台**: 合成患者风险队列（seed=42）、点击查看回填预测表单、CSV拖拽批量上传
5. **批量预测**: CSV 文件上传 → 逐行预测 → 结果 CSV 下载（UTF-8 BOM）
6. **管理仪表盘**: AKI发生率趋势、科室分布（演示数据）
7. **报告中心**: 示例报告下载、模型特征时点分组
8. **数据治理**: 治理流程图、特征缺失值/训练中位数查询、质量仪表盘

后端 API 文档（Swagger UI）：启动后访问 `/docs`。

---

## 技术栈

- **语言**: Python 3.10+ / TypeScript 5
- **数据处理**: pandas, numpy, scikit-learn
- **模型**: LogisticRegression, RandomForest, XGBoost, ExtraTrees（Voting 软集成）
- **高级对比**: Optuna, StackingClassifier, RFECV, Boruta-lite
- **补充证据**: IterativeImputer, SMOTE, DCA 阈值推荐, 重复CV不确定性
- **可解释性**: SHAP（TreeExplainer，基于XGBoost子模型）
- **训练侧可视化**: matplotlib, seaborn
- **后端**: FastAPI + uvicorn + pydantic v2 + joblib + fpdf2
- **前端**: Vue 3 + Vite + vue-router + TypeScript（图表为手写SVG/CSS，无第三方图表库）
- **验证**: RepeatedStratifiedKFold 嵌套CV、Bootstrap、Isotonic 校准

---

## 团队分工

| 角色              | 负责人       | 主要职责                 |
| ----------------- | ------------ | ------------------------ |
| 队长/技术路线     | 蓝可         | 统筹、建模、优化         |
| 数据清洗/特征工程 | 李婷、蓝可   | 数据预处理、EDA          |
| 建模/SHAP         | 梁日娇、蓝可 | 模型训练、评估、可解释性 |
| Web开发           | 王若兮       | FastAPI + Vue 3 全栈开发 |
| 报告撰写          | 叶宇晨、蓝可 | 报告撰写                 |

---

## 免责声明

本系统仅供学术研究和临床参考，不作为最终诊断依据。所有临床决策应由合格医疗专业人员根据综合患者评估做出。

## 数据与隐私

原始临床数据包含患者姓名等可识别信息，**不随本仓库或提交包分发**。训练脚本所需的 `data/raw/AKI数据.xlsx` 仅保存在受控环境；数据字典中的姓名列已脱敏，患者级逻辑校验明细 `aki_logic_validation_flags.csv` 与行级范围校验明细 `outputs/tables/clinical_range_flags.csv`（仅行号+脱敏值，无身份字段）均已移出公开仓库跟踪，仅保留汇总校验报告；如需核对 44 条范围标记，可在受控环境运行 `python run_clean.py` 重新生成。若后续需要公开数据集，必须完成彻底匿名化并取得医院伦理审批与患者知情同意。

`aki-prediction-system_提交版.zip` 为脱敏提交包，已剔除 `data/raw/`、`.git/`、缓存与临时文件。

## License

仅限学术用途
