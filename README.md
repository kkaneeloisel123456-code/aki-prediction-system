...
基于广西某三甲医院临床数据（420例），利用多种机器学习算法构建急性肾损伤（AKI）预测模型。通过 SHAP 可解释性分析 + DCA 决策曲线 + 嵌套式交叉验证 + Bootstrap 内部验证，为临床决策提供透明、可信的个体化风险评估。最终部署为 FastAPI + Vue 3 在线临床决策支持系统。
...
4. 浏览器自动打开 http://localhost:8000——（API 文档：http://localhost:8000/docs）
...
# 6. 启动 Web 系统（FastAPI + Vue 3；一键版直接双击 启动系统.bat）
...
系统为 FastAPI + Vue 3 单页应用，共 8 个页面：

1. **首页**: 核心指标、样本量/发生率、模型性能对比、功能导航
2. **模型性能**: 50次嵌套CV结果、混淆矩阵、ROC/PR、校准/DCA、SHAP、Bootstrap与消融对比
3. **风险预测** (核心): 分时段表单输入 → 实时预测 → 风险仪表 → SHAP解释 → PDF报告
4. **医生工作台**: 20例合成患者风险队列、一键回填预测表单
5. **批量预测**: CSV上传 → 逐行预测 → 结果CSV下载
6. **管理仪表盘**: AKI发生率趋势、科室分布（演示数据）
7. **报告中心**: 示例报告下载、模型特征时点分组
8. **数据治理**: 治理流程、缺失值/中位数查询、质量仪表盘
