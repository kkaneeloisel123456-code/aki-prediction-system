...
新增 `run_p1.py`，与 `run_clean.py` 共用同一套数据准备与泄漏规则；所有实验在训练折内完成，诚实报告（无选择偏倚）。
...
**折内预处理**：CV 使用未做全局填补的原始候选矩阵（49 个缺失值 = 420×87 候选矩阵经临床范围校验、置不可能值为缺失后的缺失值总数，对应 `outputs/tables/clinical_range_flags.csv` 的 44 条标记，该行级明细在受控环境生成、不随公开仓库分发），中位数填补、标准化与 RF Top35 特征选择（SelectFromModel 每折重新拟合）均在每个训练折内完成，避免“用全量数据算中位数/选特征再进 CV”的隐含泄漏；模块3 基于全量数据确定的 Top35 仅用于最终部署特征清单与 SHAP/相关性等解释分析，不参与 CV 性能评估。
