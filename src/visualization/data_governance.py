...
def plot_data_governance_flowchart(
    ...
    2. 缺失值分析 — 原始缺失5个 + 临床范围校验44个，共49个；训练折内中位数填补
    3. 异常值检测与处理 — 临床参考范围识别 + 人工审查，异常置为缺失后一并折内填补
    ...
    nodes = [
        ...
        ("② 缺失值分析", [
            "原始缺失5个 (0.01%, 4列)",
            "训练折内中位数填补 (Fold-contained Median Imputation)",
        ], _PALETTE["aqua"], 0.66),

        ("③ 异常值检测与处理", [
            "基于临床参考范围识别异常值",
            "44个临床范围异常置为缺失",
            "异常+原始缺失共49个，训练折内中位数填补",
        ...
        ("⑤ 数据泄露审查", [
            "排除KDIGO诊断标准",
            "排除结局/身份/未来信息变量",
            "97列 → 排除14列 → 83候选列 / One-Hot后87个数值特征",
        ], _PALETTE["red"], 0.24),
