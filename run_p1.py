...
# Use the raw (NaNs preserved) matrix so the per-fold imputer actually
# imputes inside CV; the previously used prep['X'] was already full-data
# median-filled, making every "honest" pipeline's imputer a no-op.
X = prepare_raw_numeric(df); y = np.asarray(prep['y'])
...
# ---------- 2) Top8/Top12 精简模型 ----------
# NOTE: feature ranking is now done INSIDE each CV fold (SelectFromModel),
# fixing the previous selection leakage where RF importance was fit on the
# full dataset before cross_val_score (reduced-model AUCs were inflated).
...
def pipe_topn(clf, n_feat):
    sel = RandomForestClassifier(n_estimators=200, class_weight='balanced', random_state=42, n_jobs=-1)
    return Pipeline([
        ('imp', SimpleImputer(strategy='median')),
        ('sc', StandardScaler()),
        ('sel', SelectFromModel(sel, max_features=n_feat, threshold=-np.inf)),
        ('clf', clf),
    ])
...
for n_feat in [8, 12, 35]:
    for name, clf in [('LR', make_lr()), ('Ridge', make_ridge())]:
        aucs = cross_val_score(pipe_topn(clf, n_feat), X, y, cv=rskf, scoring='roc_auc', n_jobs=-1)
